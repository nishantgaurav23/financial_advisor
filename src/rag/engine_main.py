...
class RAGEngine:
    ...
    def _get_chat_history(self) -> list:
        """Retrieve chat history as a list of structured message objects."""
        messages = []
        raw_messages = self.memory.chat_memory.messages  # Access raw memory messages

        # Log raw message content for debugging
        logger.info(f">>>>>>>>>>>>>>> Raw Messages: {raw_messages}")
        print(f">>>>>>>>>>>>>>> Raw Messages: {raw_messages}")

        # Process each message if it exists
        for msg in raw_messages:
            if isinstance(msg, HumanMessage):
                ...
                logger.error(f"Unexpected message type: {type(msg)} - {msg}")
        
        # Log formatted messages for confirmation
    ...
    def query(self, question: str, context: Dict = None) -> Dict:
        ...
        try:
                # Detect scenario and manage context
            is_followup = len(self.memory.chat_memory.messages) > 0
            scenario_type = self.financial_service._detect_financial_scenario(question)
            ...
            # Combine input into a single key for QA chain
            # combined_input = {
            #     "combined_input": enhanced_input,
            #     "chat_history": self.memory.chat_memory.messages
            # }
            # Retrieve structured chat history
            structured_chat_history = self._get_chat_history()
            print(f">>>>>>>>>>>>>>>>> Structured Chat History: {structured_chat_history}")
            print(f">>>>>>>>>>>>>>>>> Chat History: {self.memory.chat_memory.messages}")
            if not structured_chat_history:
                logger.error("Structured chat history is empty.")
